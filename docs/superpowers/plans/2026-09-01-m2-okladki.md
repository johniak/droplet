...
- Testy nie wykonują prawdziwego HTTP — wszystko przez `responses`.
...
- Modify: `backend/requirements.txt`, `backend/droplet/settings.py` (INSTALLED_APPS += `"covers"`)
- Test: `backend/covers/tests/test_model.py`
...
- [ ] **Step 3: Implementacja** — `startapp covers`, model jak wyżej, `covers/paths.py`:
...
def _make_thumb(game_id: int) -> None:
    img = Image.open(full_path(game_id))
...
def scan_library() -> int:
    run = run_scan()
    if run.status == ScanRun.Status.SUCCESS:
...
- [ ] **Step 2: Uruchom** — powinien przechodzić już po Task 4 (gry z okładką nie wchodzą do `match_all`); jeśli FAIL, to znaleziony bug w Task 4 — napraw tam.
...
**Interfaces:**
- Consumes: harness z M0, fixture-library z M1 (skan tworzy gry). E2e nie polega na zewnętrznej sieci (GitHub) — testuje kontrakt endpointu i auth; realne pobieranie okładek weryfikuje Task 8 na prawdziwym NAS-ie.
...
def test_cover_missing_is_404(base_url, auth):
    ...
    assert resp.status_code == 404  # fixture-library nie ma dopasowań w e2e
```
