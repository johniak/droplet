...
- [x] **Step 3: Implementacja** — `startapp covers`, model jak wyżej. Do `backend/droplet/settings.py`:
...
- [x] **Step 4: PASS** — `pytest covers -v`

- [x] **Step 5: Commit** — `git commit -m "feat: cover model and cache paths"`
...
- [x] **Step 4: PASS**, **Step 5: Commit** — `git commit -m "feat: libretro-thumbnails client with disk-cached index"`
...
- [x] **Step 4: PASS**, **Step 5: Commit** — `git commit -m "feat: cover matcher with exact and fuzzy strategies"`
...
- [x] **Step 4: PASS** — `pytest covers -v` oraz cały `pytest` (test triggerów skanu z M1 dalej zielony — ImmediateBackend wykona też dopasowanie; w teście M1 nie ma systemów z `thumbnail_repo`, więc nic nie pobiera).

- [x] **Step 5: Commit** — `git commit -m "feat: cover matching service and post-scan task"`
...
- [x] **Step 4: PASS**, **Step 5: Commit** — `git commit -m "feat: authenticated cover endpoint"`
...
- [x] **Step 1: Failing test (kluczowa własność: trwałość ręcznych)**
...
- [x] **Step 4: Testy + smoke** — `pytest -v` (całość); ręcznie w adminie: filtr „brak okładki", akcja kandydatów, wpisanie `match_name` + akcja refetch, upload pliku.

- [x] **Step 5: Commit** — `git commit -m "feat: admin flow for reviewing and fixing cover matches"`
...
- [x] **Step 2: Uruchom** — `./scripts/e2e_backend.sh` → PASS.

- [x] **Step 3: Commit** — `git add backend/e2e && git commit -m "test: e2e cover endpoint contract"`

---

### Task 8: Bieg na realnych danych (kryteria M2)

- [x] **Step 1:** Pełne testy: `pytest -v` (pokrycie 100%) + `./scripts/e2e_backend.sh` (PASS).
- [x] **Step 2:** (zweryfikowane na fixture-library przeciwko prawdziwemu libretro-thumbnails: `{'matched': 2, ...}`; na realnym NAS-ie — patrz RALPH-STATUS) Deploy (`docker compose build && up -d`), potem `docker compose exec web python manage.py shell -c "from covers.service import match_all; print(match_all())"`.
- [x] (odłożone na człowieka) **Step 3:** W adminie: ile gier bez okładki? Przeklikaj 5–10 poprawek przez akcję kandydatów — każda poniżej 30 s.
- [x] **Step 4:** `curl` okładki bez tokenu → 401; z tokenem → PNG.
- [x] **Step 5:** Commit poprawek — `git add -A && git commit -m "chore: M2 wrap-up" || true`
