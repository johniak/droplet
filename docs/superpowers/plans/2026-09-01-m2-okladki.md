...
- Obowiązują Global Constraints z M0/M1.
...
Do `backend/requirements.txt` dopisz:
...
### Task 7: Bieg na realnych danych (kryteria M2)

- [ ] **Step 1:** Deploy (`docker compose build && up -d`), potem `docker compose exec web python manage.py shell -c "from covers.service import match_all; print(match_all())"`.
- [ ] **Step 2:** W adminie: ile gier bez okładki? Przeklikaj 5–10 poprawek przez akcję kandydatów — każda poniżej 30 s.
- [ ] **Step 3:** `curl` okładki bez tokenu → 401; z tokenem → PNG.
- [ ] **Step 4:** Commit poprawek — `git add -A && git commit -m "chore: M2 wrap-up" || true`
